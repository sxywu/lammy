$("#navi").click(function(e) {
  ...
  $(".females img").tooltip({"animation": true, "placement": "right", "trigger": "hover"});
	$(".males img").tooltip({"animation": true, "placement": "right", "trigger": "hover"});
	$("#navi img").tooltip({"animation": true, "placement": "top", "trigger": "hover", "title": "click me!"});
	
$("img.pic").each(function(i, img) {
...
$(".pledge").click(function() {
  var amount = $("#pledgeamount").val(),
	loader = $("<img></img>").attr("src", "images/ajax-loader.gif")
		.attr("class", "loader");
	$("#inputs").append(loader);
  ...
  if (amount) {
    ...
    $.ajax({
        ...
        type: "POST",
        dataType: "json",
        data: {objectData: {"amount": amount}},
        success: function(data) {
          $("#inputs").children().hide();
          $("#inputs").html("thank you for donating <strong>$" + 
            amount + "</strong> <3.<br>because I don't have Paypal or Google or Amazon Wallet or anything fancy like that, I will be collecting the donations at the parteeehhh so make sure to bring enough cash (:");
        }
    });
  }
...
$("#submit").click(function(e) {
	var oneword = $("#oneword").val(),
		favmoment = $("#favmoment").val();
	$.ajax({
      url: "/submit",
      type: "POST",
      dataType: "json",
      data: {objectData: {"oneword": oneword,
			"favmoment": favmoment}},
			success: function(data) {
				if (data.redirect) {
